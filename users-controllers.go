...
func IndexUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	utills.SendJSONRespone(w, http.StatusOK, users)
}
...
func ShowUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	utills.SendJSONRespone(w, http.StatusOK, user)
}
...
func UpdateUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil && err != http.ErrMissingFile {
		...
		if err != nil {
			utills.HandleError(w, http.StatusInternalServerError, "Error saving image file: "+err.Error())
			return
		}
		user.Img = &imageName
		newImg = &imageName
	...
	if user.Img != nil {
		*user.Img = strings.TrimPrefix(*user.Img, utills.Domain+"/")
	}
	...
	query, args, err = QB.
		...
		ToSql()
	if err != nil {
		utills.DeleteImageFile(*newImg)
		utills.HandleError(w, http.StatusInternalServerError, "Error building query")
		return
	}
	...
	if oldImg != nil {
		if err := utills.DeleteImageFile(*oldImg); err != nil {
			log.Println(err)
		}
	}
...
func DeleteUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	query, args, err := QB.Delete("users").
		...
		ToSql()
	if err != nil {
		utills.HandleError(w, http.StatusInternalServerError, "Error building query: "+err.Error())
		return
	}

	var img *string
	if err := db.QueryRow(query, args...).Scan(&img); err != nil {
		utills.HandleError(w, http.StatusInternalServerError, "Error deleting user: "+err.Error())
		return
	}
	// If the user has an image, delete it
	if img != nil {
		if err := utills.DeleteImageFile(*img); err != nil {
			log.Println(err)
		}
	}
